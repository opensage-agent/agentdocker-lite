...
Drop-in replacement for the Rust _core extension module. All functions
call the `nitrobox-core` Go binary via subprocess with JSON stdin/stdout.
No in-process Go library — avoids Go runtime conflicting with Python's
asyncio event loop.
"""
...
def py_mount_overlay(
    ...
    _call("mount-overlay", {
        "lowerdir_spec": lowerdir_spec, "upper_dir": upper_dir,
        "work_dir": work_dir, "target": target, "extra_opts": [],
    })
...
def py_qmp_send(socket_path: str, command_json: str, timeout_secs: int = 30) -> str:
    return _call("qmp-send", {
        "socket_path": socket_path, "command_json": command_json,
...
def py_convert_whiteouts(layer_dir: str, use_user_xattr: bool = True) -> int:
    return _call("convert-whiteouts", {
...
def py_drop_capabilities(
    ...
    return _call("drop-capabilities", {
...
def py_apply_landlock(
    ...
    return _call("apply-landlock", {
        "read_paths": read_paths or [], "write_paths": write_paths or [],
        "allowed_tcp_ports": allowed_tcp_ports or [], "strict": strict,
    })


# ======================================================================
# Namespace operations (not available via subprocess — use CGO constructor)
# ======================================================================


def py_nsenter_preexec(target_pid: int) -> None:
    raise NotImplementedError("use CGO constructor via _NITROBOX_NSENTER env var")


def py_userns_preexec(target_pid: int, rootfs: str, working_dir: str) -> None:
    raise NotImplementedError("use CGO constructor via _NITROBOX_NSENTER env var")


def py_userns_fixup_for_delete(userns_pid: int, dir_path: str) -> int:
    return _call("userns-fixup-for-delete", {
        "userns_pid": userns_pid, "dir_path": dir_path,
    })


# ======================================================================
# Spawn — via subprocess with pass_fds
# ======================================================================
...
class PySpawnResult:
    ...
    def __init__(self, pid, stdin_fd, stdout_fd, signal_r_fd,
                 signal_w_fd_num, master_fd, pidfd, err_r_fd):
        self.pid = pid
...
def py_spawn_sandbox(config: dict) -> PySpawnResult:
    # Set PR_SET_CHILD_SUBREAPER so sandbox shell (grandchild of Go
    # subprocess) gets reparented to us. But only if we're not inside
    # an asyncio event loop (subreaper + asyncio = broken self-pipe).
    import asyncio
    try:
        asyncio.get_running_loop()
        # Inside event loop — skip subreaper (harbor uses run_in_executor)
    except RuntimeError:
        # No event loop — safe to set subreaper
        import ctypes
