...
const SECCOMP_RET_ENOSYS: u32 = SECCOMP_RET_ERRNO | 38;

const AUDIT_ARCH_X86_64: u32 = 0xC000_003E;
#[cfg(target_arch = "aarch64")]
const AUDIT_ARCH_AARCH64: u32 = 0xC000_00B7;

const CLONE_NS_FLAGS: u32 = 0x0002_0000 | 0x0400_0000 | 0x0800_0000
    | 0x1000_0000 | 0x2000_0000 | 0x4000_0000 | 0x0000_0080;
...
static ARCH: ArchConfig = ArchConfig {
    ...
    blocked: &[
        (101, "ptrace"), (165, "mount"), (166, "umount2"), (155, "pivot_root"),
        (272, "unshare"), (308, "setns"),
        ...
        (167, "swapon"), (168, "swapoff"), (163, "acct"),
        (310, "process_vm_readv"), (311, "process_vm_writev"),
        (304, "open_by_handle_at"), (303, "name_to_handle_at"),
        (321, "bpf"), (298, "perf_event_open"), (323, "userfaultfd"),
        (250, "keyctl"), (248, "add_key"), (249, "request_key"),
        (173, "ioperm"), (172, "iopl"),
        (425, "io_uring_setup"), (426, "io_uring_enter"), (427, "io_uring_register"),
    ],
    clone_nr: 56, clone3_nr: 435, ioctl_nr: 16,
};
...
static ARCH: ArchConfig = ArchConfig {
    ...
    blocked: &[
        (117, "ptrace"), (40, "mount"), (39, "umount2"), (41, "pivot_root"),
        (97, "unshare"), (268, "setns"),
        (105, "init_module"), (273, "finit_module"), (106, "delete_module"),
        (104, "kexec_load"), (294, "kexec_file_load"),
        (142, "reboot"), (161, "sethostname"), (162, "setdomainname"),
        (224, "swapon"), (225, "swapoff"), (89, "acct"),
        (270, "process_vm_readv"), (271, "process_vm_writev"),
        (265, "open_by_handle_at"), (264, "name_to_handle_at"),
        (280, "bpf"), (241, "perf_event_open"), (282, "userfaultfd"),
        (219, "keyctl"), (217, "add_key"), (218, "request_key"),
        (425, "io_uring_setup"), (426, "io_uring_enter"), (427, "io_uring_register"),
    ],
    clone_nr: 220, clone3_nr: 435, ioctl_nr: 29,
};
...
pub fn build_seccomp_bpf() -> Vec<u8> {
    ...
    let ptr = insns.as_ptr() as *const u8;
...
pub fn apply_seccomp_filter() -> io::Result<()> {
    ...
    log::debug!("seccomp: blocked {} syscalls + clone NS flags + ioctl TIOCSTI", ARCH.blocked.len());
    Ok(())
}
...
fn add_path_rule(ruleset_fd: RawFd, path: &str, access: u64) -> io::Result<()> {
    ...
    unsafe {
        ll_syscall(libc::SYS_landlock_add_rule, &[
            ruleset_fd as usize, LANDLOCK_RULE_PATH_BENEATH as usize,
            &rule as *const _ as usize, 0,
        ]);
        libc::close(fd);
    }
    Ok(())
...
pub fn apply_landlock(
    ...
    if abi == 0 {
        ...
        if strict { return Err(io::Error::new(io::ErrorKind::Unsupported, msg)); }
        log::debug!("{}", msg);
        return Ok(false);
    }
    ...
    if !read_paths.is_empty() { handled_fs |= FS_READ; }
    if !write_paths.is_empty() {
        if read_paths.is_empty() { handled_fs |= write_only_mask; }
        else { handled_fs |= fs_write; }
    }
    ...
    let attr = LandlockRulesetAttr { handled_access_fs: handled_fs, handled_access_net: handled_net, scoped };
    let ruleset_fd = unsafe {
        ll_syscall(libc::SYS_landlock_create_ruleset, &[
            &attr as *const _ as usize, std::mem::size_of::<LandlockRulesetAttr>(), 0,
        ])
    };
    if ruleset_fd < 0 {
        let msg = format!("landlock_create_ruleset failed: {}", io::Error::last_os_error());
        if strict { return Err(io::Error::new(io::ErrorKind::Other, msg)); }
        log::warn!("{}", msg);
        return Ok(false);
    }
    ...
    let result = (|| -> io::Result<bool> {
        ...
        for &port in allowed_tcp_ports {
            if abi < 4 { break; }
            let rule = LandlockNetPortAttr { allowed_access: LANDLOCK_ACCESS_NET_CONNECT_TCP, port: port as u64 };
            unsafe { ll_syscall(libc::SYS_landlock_add_rule, &[
                ruleset_fd as usize, LANDLOCK_RULE_NET_PORT as usize,
                &rule as *const _ as usize, 0,
            ]); }
        }

        // NO_NEW_PRIVS via rustix
        rustix::thread::set_no_new_privs(true).map_err(|e| io::Error::from_raw_os_error(e.raw_os_error()))?;

        let mut restrict_flags: u32 = 0;
        if abi >= 7 { restrict_flags |= LANDLOCK_RESTRICT_SELF_LOG_NEW_EXEC_ON; }
        if abi >= 8 { restrict_flags |= LANDLOCK_RESTRICT_SELF_TSYNC; }

        let mut ret = unsafe { ll_syscall(libc::SYS_landlock_restrict_self, &[ruleset_fd as usize, restrict_flags as usize]) };
        if ret < 0 && (restrict_flags & LANDLOCK_RESTRICT_SELF_TSYNC) != 0 {
            restrict_flags &= !LANDLOCK_RESTRICT_SELF_TSYNC;
            ret = unsafe { ll_syscall(libc::SYS_landlock_restrict_self, &[ruleset_fd as usize, restrict_flags as usize]) };
        }
        if ret < 0 {
            let msg = format!("landlock_restrict_self failed: {}", io::Error::last_os_error());
            if strict { return Err(io::Error::new(io::ErrorKind::Other, msg)); }
            log::warn!("{}", msg);
            return Ok(false);
        }

        log::debug!("Landlock (ABI v{}): {} read, {} write paths, {} TCP ports",
            abi, read_paths.len(), write_paths.len(), allowed_tcp_ports.len());
        Ok(true)
    })();
